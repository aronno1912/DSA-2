...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String name[] = new String[ver];//team
        int win[] = new int[ver];
        int loss[] = new int[ver];
        int left[] = new int[ver];
        int capacity[][]=new int[ver][ver]; //koyta jita lagbe
        int game[][] = new int[ver][ver];//against
        ...
        //team er vertex theke edge form kortesi
        for(int i = 0; i < ver; i++){
            Graph g = new Graph(nodes);
            int teamInd[] = new int[ver];
            int curNode =( (ver - 1) * (ver - 2) / 2) + 1; //first node of team
            for(int j = 0; j < ver; j++){
                if(i == j)continue;
                teamInd[j] = curNode;
                g.addEgde(curNode, nodes - 1, capacity[i][j]); //nodes -1 means tank
                curNode++;
            }
            curNode = 1; //first index of game
            int total = 0;
            for(int j = 0; j < ver; j++){
                if(i == j)continue;
